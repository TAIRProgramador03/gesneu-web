...
interface ModalConfirmarInspeccionProps {
    open: boolean;
    fechaUltimaInspeccion: string;
    diasDiferencia?: number;
    onClose: () => void;
    onRegistrarInspeccion: () => void;
    onContinuarReubicacion: () => void;
}
...
const ModalConfirmarInspeccion: React.FC<ModalConfirmarInspeccionProps> = ({
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(fechaUltimaInspeccion)) {
        ...
        // Asegura dos dígitos en día y mes
    ...
    else if (esHoy) mensaje = `Ya se realizó una inspección hoy (${fechaFormateada}). ¿Desea continuar con la reubicación?`;
