...
export function CustomersFilters({
  ...
  const [displayBajaDefinitiva, setDisplayBajaDefinitiva] = React.useState(0);
  const [displayRecuperados, setDisplayRecuperados] = React.useState(0);
  ...
  // Animación baja definitiva
  React.useEffect(() => {
    let start = 0;
    let rafId: number;
    const animate = () => {
      const increment = Math.ceil((bajaDefinitivaCount - start) / 20);
      start += increment;
      if (start >= bajaDefinitivaCount) {
        setDisplayBajaDefinitiva(bajaDefinitivaCount);
      } else {
        setDisplayBajaDefinitiva(start);
        rafId = requestAnimationFrame(animate);
      }
    };
    if (bajaDefinitivaCount > 0) {
      setDisplayBajaDefinitiva(0);
      rafId = requestAnimationFrame(animate);
    } else {
      setDisplayBajaDefinitiva(0);
    }
  ...
  // Animación recuperados
  React.useEffect(() => {
    let start = 0;
    let rafId: number;
    const animate = () => {
      const increment = Math.ceil((recuperadosCount - start) / 20);
      start += increment;
      if (start >= recuperadosCount) {
        setDisplayRecuperados(recuperadosCount);
      } else {
        setDisplayRecuperados(start);
        rafId = requestAnimationFrame(animate);
      }
    };
    if (recuperadosCount > 0) {
      setDisplayRecuperados(0);
      rafId = requestAnimationFrame(animate);
    } else {
      setDisplayRecuperados(0);
    }
