...
const MOCK_UBICACIONES: { [key: string]: number[] } = {
  'TOQUEPALA': [-17.2452, -70.6212],
  'CUAJONE': [-17.0319, -70.7108],
  'CHUNGAR': [-11.0267, -76.4319],
  'TAIR LIMA': [-12.0464, -77.0428],
  'ILO': [-17.6433, -71.3411],
  'MARCONA': [-15.1764, -75.1219],
  'TAIR AREQUIPA': [-16.4090, -71.5375],
  'ANTAMINA': [-9.5447, -77.0519],
  'ANDAYCHAGUA': [-11.5333, -75.9333],
}
...
function MapFlyTo({ taller }: { taller: TallerConNeumaticos | null }) {
  ...
  React.useEffect(() => {
    if (taller) {
      map.flyTo((MOCK_UBICACIONES[taller.TALLER] || [-9.5, -75.5]) as [number, number], 10, { animate: true, duration: 1.2 });
    } else {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, flexShrink: 0 }}>
        {([
          ...
          { label: 'Talleres con flota', value: talleresConNeumaticos.length, sub: 'en todo el Perú', color: '#8B5CF6' },
        ...
        <div style={{ flex: 1, borderRadius: 12, overflow: 'hidden', border: `1px solid ${border}` }}>
          ...
          >
            ...
            {talleresConNeumaticos.map((taller) => {
              ...
              const radius = getRadius(taller.CANTIDAD_NEUMATICOS);
