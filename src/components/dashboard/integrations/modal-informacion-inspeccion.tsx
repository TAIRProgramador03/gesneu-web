...
import { columnsNeuPorAsignar, columnsNeuPorInspeccionar } from '@/app/(app)/integrations/columns';
...
export const ModalInformacionInspeccion = ({ open, kilometraje, fechaInspeccion, neumaticos = [], onClose, onSuccessInspeccion, placa }: ModalVerInspeccionesProps) => {
      ...
      <DialogTitle sx={{ pb: 1.5, pt: 2, display: 'flex', alignItems: 'center', gap: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Typography variant="caption" className='text-amber-600' sx={{ display: 'block', mt: 1, fontStyle: 'italic' }}>
            ...
            Aquí podrás visualizar los datos para tu inspección vehicular. Confirmar que la información mostrada es la correcta.
            <br />
            Para guardar la inspección: <b>Registrar Inspección</b>
          </Typography>
        </Box>
      </DialogTitle>
      ...
      <DialogContent sx={{ pt: 2, bgcolor: '#f8fafc' }}>
        ...
        <Card sx={{ flex: 1, p: 2.5, borderRadius: 2.5, border: '1px solid #e2e8f0', marginTop: '10px' }} elevation={0}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
            ...
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-teal-100 text-teal-600 text-sm font-normal">
              <Tally1 size={16} />
              Fecha de inspección:
              <b>{fechaInspeccion ? convertToDateHuman(fechaInspeccion) : '-'}</b>
            </span>
          </Box>
          <DataTableNeumaticos columns={columnsNeuPorInspeccionar} data={neumaticos} />
        </Card>
      </DialogContent>
